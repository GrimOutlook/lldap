...
async fn migrate_to_v12(transaction: DatabaseTransaction) -> Result<DatabaseTransaction, DbErr> {
    let builder = transaction.get_database_backend();
    // Add is_system to users table, for the read-only system accounts
    // feature -- distinguishes accounts managed declaratively (via
    // system_accounts config) from normal user-editable accounts.
    transaction
        .execute(
            builder.build(
                Table::alter().table(Users::Table).add_column(
                    ColumnDef::new(Users::IsSystem)
                        .boolean()
                        .not_null()
                        .default(false),
                ),
